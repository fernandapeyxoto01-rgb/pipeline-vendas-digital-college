...
# ── CONEXÃO ──────────────────────────────────────────────────────────────────
db_url = os.getenv("DASHBOARD_DATABASE_URL")
if not db_url:
    raise ValueError("DASHBOARD_DATABASE_URL não definida no .env")

engine_dm = create_engine(db_url)

def carregar_dados() -> pd.DataFrame:
    df = pd.read_sql("""
        SELECT ano, mes, qtde_vendida, valor_total_real, valor_total_esperado
        FROM public.vendas_ano_mes_fernanda
        ORDER BY ano, mes
    """, engine_dm)
...
def carregar_localidade() -> pd.DataFrame:
    df = pd.read_sql("""
        SELECT *
        FROM public.vendas_localidade_fernanda
    """, engine_dm)
    return df

DF_LOCALIDADE = carregar_localidade()
...
app.layout = html.Div(style={
    ...
    html.Div(style={"padding": "28px 32px"}, children=[
        ...
        html.Div(style={
            ...
            graph_card("Top Estados por Receita", "g-estado"),
            graph_card("Top Cidades por Receita", "g-cidade"),
        ]),
...
def atualizar(ano):
    ...
    df_loc = DF_LOCALIDADE[DF_LOCALIDADE["ano"] == ano].copy()
    ...
    df_estado = (
        ...
        .agg({"valor_total_real": "sum"})
        .sort_values("valor_total_real", ascending=False)
        .head(10)
    )
    fig_estado = go.Figure(layout=go.Layout(**plot_layout()))
    fig_estado.add_trace(go.Bar(
        x=df_estado["valor_total_real"],
        y=df_estado["sigla_estado"],
        orientation="h",
        marker_color=ORANGE,
        hovertemplate="%{x:,.0f}<extra>%{y}</extra>",
    ))
    fig_estado.update_layout(yaxis=dict(autorange="reversed"))

    # ── G6: Top Cidades ──────────────────────────────────────────────────────
    df_cidade = (
        df_loc.sort_values("valor_total_real", ascending=False)
        .head(10)
    )
    fig_cidade = go.Figure(layout=go.Layout(**plot_layout()))
    fig_cidade.add_trace(go.Bar(
        x=df_cidade["cidade"],
        y=df_cidade["valor_total_real"],
        marker_color=ORANGE2,
        hovertemplate="%{y:,.0f}<extra>%{x}</extra>",
    ))

    return cards, fig_rec, fig_at, fig_qt, fig_dev, fig_estado, fig_cidade
